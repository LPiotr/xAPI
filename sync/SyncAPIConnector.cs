...
// Type: xAPI.Sync.SyncAPIConnector
...
namespace xAPI.Sync
{
  public class SyncAPIConnector : Connector
  {
    ...
    public event SyncAPIConnector.OnConnectedCallback OnConnected;

    public event SyncAPIConnector.OnRedirectedCallback OnRedirected;
    ...
    private void Connect(Server server, bool lookForBackups = true)
    {
      ...
      if (server.Secure)
      {
        ...
        if (!ExecuteWithTimeLimit.Execute(TimeSpan.FromMilliseconds(5000.0), (Action) (() => sl.AuthenticateAsClient(server.Address, new X509CertificateCollection(), SslProtocols.Default, false))))
          throw new APICommunicationException("Error during SSL handshaking (timed out?)");
        this.apiWriteStream = new StreamWriter((Stream) sl);
        this.apiReadStream = new StreamReader((Stream) sl);
      }
      else
      {
